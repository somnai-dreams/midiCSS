...
(() => {
  ...
  const TAGS = {
    ...
    H3: { wave: "square", vol: 0.22, maxDur: 1 },
  ...
  const collect = () => {
    ...
    for (const el of document.querySelectorAll(SELECTOR)) {
      ...
      const chroma = (Math.max(a, b, c) - Math.min(a, b, c)) / 255;
      notes.push({
        el,
        t,
  ...
  const timer = window.setInterval(() => {
    if (ctx.currentTime > scheduledThrough - 0.3)
      schedulePass(scheduledThrough);
  }, 120);
  const ph = document.createElement("div");
  ph.style.cssText = "position:fixed;top:0;bottom:0;left:0;width:2px;background:#f0a;box-shadow:0 0 8px #f0a;z-index:2147483647;pointer-events:none";
  ...
  console.log("%cmidiCSS sonify: listening to this page — scroll to remix · Escape to stop · paste again to toggle", "color:#f0a;font-weight:bold");
})();
